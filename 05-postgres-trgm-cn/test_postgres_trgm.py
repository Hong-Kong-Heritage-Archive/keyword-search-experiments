...
def search_similar_titles(query_term="Harry Potter"):
    ...
    print(f"Explain searching for titles similar to '{query_term}'...")
    ...
    try:
        ...
        cursor.execute(f"""
        EXPLAIN SELECT title, isbn, similarity(title, '{query_term}') AS similarity_score
        FROM books
